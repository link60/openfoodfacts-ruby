...
class TestOpenfoodfacts < Minitest::Test
  ...
  def test_it_fetches_products_for_state
    product_state = ::Openfoodfacts::ProductState.new(
      'url' => 'https://world.openfoodfacts.org/facets/states/photos-uploaded', 'products_count' => 22
    )
  ...
  def test_it_fetches_products_for_number_of_ingredients
    ...
    VCR.use_cassette('products_for_number_of_ingredients', record: :once, match_requests_on: %i[host path]) do
